...
class PaperExchange:
    ...
    def _add_indicators(self, df):
        ...
        # Volatility spike — ATR vs 20-period ATR average
        df['atr_avg_20']     = df['atr'].rolling(20).mean()
        df['volatility_spike'] = df['atr'] > df['atr_avg_20'] * 1.8

        drop = ['tr','dm_plus','dm_minus','atr14','sm_dm_plus','sm_dm_minus','dx']
    ...
    def get_latest_indicators(self, df):
        ...
        return {
            ...
            "volatility_spike": bool(row['volatility_spike']),
        }
    ...
    def open_position(self, symbol, side, qty, tp, sl, atr=None, entry_notional=None):
        ...
        self.position = {
            ...
            "tp":               tp,
            "sl":               sl,
            "original_sl":      sl,       # for trailing stop reference
            "trail_activated":  False,    # trailing stop flag
            "trail_high":       entry,    # highest price seen (long) / lowest (short)
            "atr_at_entry":     atr,
    ...
    def check_tp_sl(self, price) -> Tuple[bool, Optional[str], float]:
        """
        Check TP/SL. Also updates trailing stop if activated.
        Returns (closed, reason, pnl).
        """
        ...
        if pos.get('trail_activated'):
            # Update trail high/low and ratchet SL
            if side == 'long':
                if price > pos['trail_high']:
                    pos['trail_high'] = price
                # Trail SL = 1.5 ATR below current high
                atr  = pos.get('atr_at_entry') or (pos['entry_price'] * 0.012)
                new_sl = pos['trail_high'] - atr * 1.5
                if new_sl > pos['sl']:
            ...
            # Check if trailing stop should activate (profit >= 15%)
            if side == 'long':
        ...
        if side == 'long':
            ...
            elif price <= pos['sl']: reason, hit = ("Trailing SL 🔒" if pos.get('trail_activated') else "SL 🛑"), True
        else:
            if price <= pos['tp']:   reason, hit = "TP ✅", True
            elif price >= pos['sl']: reason, hit = ("Trailing SL 🔒" if pos.get('trail_activated') else "SL 🛑"), True

        if not hit:
            return False, ("TRAIL_ACTIVATED" if trail_just_activated else None), 0.0

        return self._close_position(price, reason)

    def close_now(self, price) -> Tuple[bool, str, float]:
        if not self.position:
            return False, "No open position.", 0.0
        return self._close_position(price, "Manual close 🤙")

    def _close_position(self, price, reason) -> Tuple[bool, str, float]:
        pos = self.position
